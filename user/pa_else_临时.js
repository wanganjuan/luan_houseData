...
function _table(trList) {
  ...
  _foreach(trList, (element, index) => {
    if (index > baseRoomIndex) {
      ...
      var num = _this.eq(0 + buffer).text().replace(/\s+/g, '')
      var rBuildArea = _this.eq(1+ buffer).text().replace(/\s+/g, '')
      var rUseArea = _this.eq(2+ buffer).text().replace(/\s+/g, '')
      var rCommonArea = _this.eq(3+ buffer).text().replace(/\s+/g, '')
      var price = _this.eq(4+ buffer).text().replace(/\s+/g, '')
      var totalPrice = _this.eq(5+ buffer).text().replace(/\s+/g, '')
      var display = _this.eq(6+ buffer).text().replace(/\s+/g, '')
      var decorate = _this.eq(7+ buffer).text().replace(/\s+/g, '')
      var remark = _this.eq(8+ buffer).text().replace(/\s+/g, '')
      // 判断是不是在一起的没有区分table
      ...
      if (_length === 1 && _this.eq(0).text().replace(/\s+/g, '').includes('价格备案表')) {
        if (baoyeTag) {
          let baseInfo = trList.eq(index + 3).find('td').eq(0).text().replace(/\s+/g, '')
          let baseReg = /备案楼号：(.*)备案时间：(.*)/
          buildNum = baseInfo.replace(baseReg, '$1').replace(/\s+/g, '')
          time = baseInfo.replace(baseReg, '$2').replace(/\s+/g, '')
          baseInfo = trList.eq(index + 4).find('td').eq(0).text().replace(/\s+/g, '')
          baseReg = /备案面积：(.*)平均价格：(.*)/
          buildArea = baseInfo.replace(baseReg, '$1').replace(/\s+/g, '')
          unitPrice = baseInfo.replace(baseReg, '$2').replace(/\s+/g, '')
        } else {
