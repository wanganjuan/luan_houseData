...
function _table(trList) {
  ...
  let c14Tag = false
  // 判断 
  if (_index === 13 && trList.eq(0).find('td').eq(0).text().replace(/\s+/g,"") && trList.eq(0).find('td').eq(0).text().replace(/\s+/g,"").includes('宝业.君悦绿苑')) {
    c13Tag = true
  }
  if (_index === 14 && trList.eq(1).find('td').eq(2).text().replace(/\s+/g,"") === '徽盐.龙湖湾') {
    c14Tag = true
  }
  // 企业
  var company = trList.eq(hasNoHeader ? 0 : 1).find('td').eq(1).text().replace(/\s+/g,"")
  // 为空说明有问题
  let tag = false
  // 缺失第二个
  ...
  var buildName = trList.eq(hasNoHeader ? 0 : 1).find('td').eq(3).text().replace(/\s+/g,"")
  ...
  if (_index === 10 && trList.eq(1).find('td').eq(5).text().replace(/\s+/g,"")) {
  ...
  // 位置
  var positionDesc = trList.eq(hasNoHeader ? 1 : 2).find('td').eq(1).text().replace(/\s+/g,"")
  var positionArea = trList.eq(hasNoHeader ? 1 : 2).find('td').eq(3).text().replace(/\s+/g,"")
  if (trList.eq(2).find('td').eq(4).text().replace(/\s+/g,"")) {
    positionArea = trList.eq(2).find('td').eq(4).text().replace(/\s+/g,"")
  }
  if (lostSecondTag) {
    positionDesc = trList.eq(hasNoHeader ? 1 : 2).find('td').eq(2).text().replace(/\s+/g,"")
    positionArea = trList.eq(hasNoHeader ? 1 : 2).find('td').eq(4).text().replace(/\s+/g,"")
  }
  if (tag) {
  ...
  // 备案楼号： 时间
  var buildNum = trList.eq(hasNoHeader ? 2 : 3).find('td').eq(1).text().replace(/\s+/g,"")
  // 上次备案时间：
  var time = trList.eq(hasNoHeader ? 2 : 3).find('td').eq(3).text().replace(/\s+/g,"")
  ...
  if (lostSecondTag) {
    buildNum = trList.eq(hasNoHeader ? 2 : 3).find('td').eq(2).text().replace(/\s+/g,"")
    // 上次备案时间：
    time = trList.eq(hasNoHeader ? 2 : 3).find('td').eq(4).text().replace(/\s+/g,"")
  }
  if (tag) {
  ...
  // 备案面积  价格
  var buildArea = trList.eq(hasNoHeader ? 3 : 4).find('td').eq(1).text().replace(/\s+/g,"")
  var unitPrice = trList.eq(hasNoHeader ? 3 : 4).find('td').eq(3).text().replace(/\s+/g,"")
  if (!hasNoHeader && trList.eq(4).find('td').eq(4).text().replace(/\s+/g,"")) {
    unitPrice = trList.eq(4).find('td').eq(4).text().replace(/\s+/g,"")
  }
  if (lostSecondTag) {
    buildArea = trList.eq(hasNoHeader ? 3 : 4).find('td').eq(2).text().replace(/\s+/g,"")
    unitPrice = trList.eq(hasNoHeader ? 3 : 4).find('td').eq(4).text().replace(/\s+/g,"")
  }
  if (hasLastTag) {
  ...
  // room
   ...
   _foreach(trList, (element, index) => {
     if (index > baseRoomIndex) {
       ...
       var num = _this.eq(0).text().replace(/\s+/g,"")
       var rBuildArea = _this.eq(1).text().replace(/\s+/g,"")
       var rUseArea = _this.eq(2).text().replace(/\s+/g,"")
       var rCommonArea = _this.eq(3).text().replace(/\s+/g,"")
       var price = _this.eq(4).text().replace(/\s+/g,"")
       var totalPrice = _this.eq(5).text().replace(/\s+/g,"")
       var display = _this.eq(6).text().replace(/\s+/g,"")
       var decorate = _this.eq(7).text().replace(/\s+/g,"")
       var remark = _this.eq(8).text().replace(/\s+/g,"")
       if (hasLastTag) {
        rUseArea = '重新备案'
        rCommonArea  = '重新备案'
      ...
      .then((buildRes) => {
        ...
        //  console.log(dongList)
         for (let dong of dongList) {
          ...
          dong.page = _index
          dong.uprice = dong.uprice.replace(/[^0-9^\.]/ig,"") || '0'
